...
function ampy_render_ev_lead_magnet( $id_or_slug ): string {
    ...
    /* Self-contained font loading for ALL THREE families the design names:
       --font-heading = Plus Jakarta Sans, --font-body = Outfit, --font-mono =
       JetBrains Mono. (Earlier this injected JetBrains Mono ONLY and relied on the
       host theme serving Jakarta + Outfit — an undocumented dependency that made
       headings/body fall back to system-ui on any non-Core-Framework page.) The
       Google URL is verbatim from the prototype; token values are unchanged — this
       only guarantees the families the --font-* tokens already name resolve. If a
       local JetBrains Mono woff2 is present it is used for the mono (progressive
       upgrade) and only Jakarta + Outfit are pulled from Google. */
    ...
    $font_preconnect = '<link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>';
    $font_html = file_exists( $mono_local_path )
        ...
        : $font_preconnect
          . '<link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700&family=Outfit:wght@400;500;600;700&family=JetBrains+Mono:ital,wght@0,100..800;1,100..800&display=swap" rel="stylesheet">';
