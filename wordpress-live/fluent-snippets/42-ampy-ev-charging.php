...
* @tags: 
* @group: 
* @name: Ampy EV Calculator - Backend
* @type: PHP
* @status: published
* @created_by: 13
* @created_at: 2026-06-18 08:22:05
* @updated_at: 2026-07-09 11:17:14
* @is_valid: 1
* @updated_by: 13
* @priority: 10
* @run_at: all
...
define( 'AMPY_EV_CALC_POST_ID', 56969 );
...
add_action( 'add_meta_boxes', function ( $post_type, $post ) {
    // Bind this box to the EV lead-magnet ONLY. Registering it for the whole
    // 'lead-magnet' type puts an empty "EV Charging Calculator" box on every
    // other lead-magnet post. Gating at registration means foreign posts never
    // see it. AMPY_EV_CALC_POST_ID = 0 falls back to "every lead-magnet".
    if ( $post_type !== 'lead-magnet' ) return;
    if ( AMPY_EV_CALC_POST_ID > 0 && (int) $post->ID !== (int) AMPY_EV_CALC_POST_ID ) return;
    add_meta_box(
...
function ampy_render_ev_lead_magnet( $id_or_slug, $preselect_charger = '' ): string {
    ...
    if ( ! $post || $post->post_type !== 'lead-magnet' ) {
        return '';
    }
    // Published tools render for everyone. Drafts/pending/private render only for
    // users who can edit the post, so you can build and preview before going live.
    ...
    /* --- Per-page charger preselect ----------------------------------------
       Product (laddbox) pages embed this calculator and should open on their
       own charger. Priority: explicit [ampy_ev_lead_magnet charger="id"] >
       the charger whose learn_more_url points at the page being rendered >
       the global default from the calculator metabox. The result is a
       charger_id; the engine validates it against CHARGERS and availability and
       falls back to the first available charger, so a miss here is always safe. */
    $charger_rows = ( isset( $js_data['CHARGERS'] ) && is_array( $js_data['CHARGERS'] ) ) ? $js_data['CHARGERS'] : array();
    $charger_ids  = array();
    foreach ( $charger_rows as $c ) { if ( ! empty( $c['id'] ) ) $charger_ids[] = (string) $c['id']; }

    $resolved_charger = '';
    if ( $preselect_charger !== '' && in_array( (string) $preselect_charger, $charger_ids, true ) ) {
        $resolved_charger = (string) $preselect_charger;
    }
    if ( $resolved_charger === '' && ! $on_own_page && is_singular() ) {
        $here_id = (int) get_queried_object_id();
        if ( $here_id ) {
            $here_url  = untrailingslashit( strtok( (string) get_permalink( $here_id ), '?#' ) );
            $here_slug = sanitize_title( (string) get_post_field( 'post_name', $here_id ) );
            foreach ( $charger_rows as $c ) {
                $cid = (string) ( $c['id'] ?? '' );
                if ( $cid === '' ) continue;
                $url       = (string) ( $c['slug'] ?? '' );
                $url_norm  = ( $url && $url !== '#' ) ? untrailingslashit( strtok( $url, '?#' ) ) : '';
                $url_last  = $url_norm ? sanitize_title( basename( (string) ( parse_url( $url_norm, PHP_URL_PATH ) ?: $url_norm ) ) ) : '';
                if ( ( $url_norm !== '' && $url_norm === $here_url )
                  || ( $url_last !== '' && $url_last === $here_slug )
                  || ( sanitize_title( $cid ) === $here_slug ) ) {
                    $resolved_charger = $cid;
                    break;
                }
            }
        }
    ...
    if ( $resolved_charger !== '' ) {
        $default_charger_id = $resolved_charger;
    }
    ...
    $font_html = $faces ? '<style>' . $faces . '</style>' : '';
...
<!-- Rem scale: the whole calculator assumes 1rem=10px (tokens, type, spacing, radii,
     the fluid --fs-* clamps). rem is ROOT-relative, so it must be re-anchored on
     <html>, not on a wrapper — the earlier `.ampy-calc-outer{font-size:62.5%}` was a
     NO-OP for rem and left the calculator at 1.6x on any host whose root is not 62.5%.
     We re-anchor the root but PAGE-GATE it with :has(), so it only fires on pages that
     actually contain the calculator — unrelated pages are never touched. On ampy.se
     (Core Framework already sets the global root to 62.5%) this is redundant-but-
     harmless. Do NOT "fix" scale by editing rem or --fs-* values — the anchor is this
     one rule. :has() is universally supported in current browsers. -->
<style>html:has(.ampy-calc-outer){font-size:62.5%;}</style>

<div class="ampy-calc-outer ampy-ev">
        ...
        <div class="ampy-calc__card ampy-calc__card--surface" aria-label="Resultat">
          ...
          <div class="ampy-calc__monthly" id="ampyEvMonthly">
            ...
            <div class="ampy-calc__monthly-cols">
              ...
              <!-- Home charging = smart charging only (owner decision 2026-06-13):
                   the flat-rate home bar was removed; this single home bar IS the
                   smart / scheduled-on-low-price-hours option. Solid (primary) styling. -->
...
add_shortcode( 'ampy_ev_lead_magnet', function ( $atts ): string {
    $atts       = shortcode_atts( [ 'slug' => '', 'id' => '', 'charger' => '' ], $atts, 'ampy_ev_lead_magnet' );
    $identifier = $atts['id'] ?: $atts['slug'];
    if ( ! $identifier ) return '';
    return ampy_render_ev_lead_magnet( $identifier, $atts['charger'] );
} );